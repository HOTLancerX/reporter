 ...
 *
 * Reporters post to the existing "blog" type.
 * The admin sets a per-reporter default post status (draft / published) in
 * UserInfo as reporter_post_status. This is passed to PostForm as defaultStatus
 * so new posts start with the correct status. Edit mode always loads from DB.
 */
...
export default function ReporterPostForm() {
    ...
    // Per-reporter default post status set by admin in /admin/reporter
    const [defaultStatus, setDefaultStatus] = useState<"draft" | "published">("draft");
    const [statusReady,   setStatusReady]   = useState(false);

    // Role guard
    useEffect(() => {
        if (user && user.type !== "reporter") router.replace("/account");
    }, [user, router]);

    // Load reporter_post_status from UserInfo
    useEffect(() => {
        if (!user?._id) return;
        ...
        fetch(`${EXPRESS_API}/user-info?userId=${user._id}`, {
            ...
            .then((data) => {
                ...
                setDefaultStatus(val === "published" ? "published" : "draft");
            })
            .catch(() => {})
    ...
    return (
        <div className="space-y-6">
            ...
            <PostForm
                ...
                onSuccess={(savedId) => {
                    if (isNew) router.replace(`/account/post/blog/${savedId}`);
                }}
            />
        </div>
    );
